...
parser.add_argument('--paraboloid', default=False,
                   help='Inserts a paraboloid neuron layer before the output layer')
parser.add_argument('--overfit', default=False,
                   help='Used with --paraboloid (and --paraboloidout) and applies the respective overfitting recipe')
...
def main():
    ...
    if (args.paraboloid):
      if (args.paraboloidout):
        if (args.overfit):
          model.fc = gpt.ParaboloidOutput(model.fc.in_features, model.fc.out_features, h_factor = 0.01, lr_factor = 1., wd_factor = 1., grad_factor = 1., input_factor = 1.0, output_factor = 0.1, p_factor=0.0001, init = 'spotlight')
        else:
        ...
        if (args.overfit):
          model.fc = nn.Sequential(gpt.Paraboloid(model.fc.in_features, 1024, h_factor = 0.01, lr_factor = 10., wd_factor = 1., grad_factor = 1., input_factor = 0.1, output_factor = 0.1, p_factor=0.0001, init = 'live'), nn.Linear(1024, model.fc.out_features))
        else:
    ...
    if (args.paraboloid or args.overfit):
